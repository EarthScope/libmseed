...
TEST (read, v3_encodings)
{
  ...
  /* Text */
  rv = ms3_readmsr (&msr, "data/reference-testdata-text.mseed3", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  ...
  /* Float32 */
  rv = ms3_readmsr (&msr, "data/reference-testdata-float32.mseed3", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Float64/double */
  rv = ms3_readmsr (&msr, "data/reference-testdata-float64.mseed3", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Int16 */
  rv = ms3_readmsr (&msr, "data/reference-testdata-int16.mseed3", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Int32 */
  rv = ms3_readmsr (&msr, "data/reference-testdata-int32.mseed3", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Steim-1 big endian */
  rv = ms3_readmsr (&msr, "data/reference-testdata-steim1.mseed3", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Steim-2 big endian */
  rv = ms3_readmsr (&msr, "data/reference-testdata-steim2.mseed3", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
...
TEST (read, v2_encodings)
{
  ...
  /* Text */
  rv = ms3_readmsr (&msr, "data/reference-testdata-text.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* CDSN */
  rv = ms3_readmsr (&msr, "data/testdata-encoding-CDSN.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* DWWSSN */
  rv = ms3_readmsr (&msr, "data/testdata-encoding-DWWSSN.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* SRO */
  rv = ms3_readmsr (&msr, "data/testdata-encoding-SRO.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* GEOSCOPE */
  rv = ms3_readmsr (&msr, "data/testdata-encoding-GEOSCOPE-16bit-3exp-encoded.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Float32 */
  rv = ms3_readmsr (&msr, "data/reference-testdata-float32.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Float64/double */
  rv = ms3_readmsr (&msr, "data/reference-testdata-float64.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Int16 */
  rv = ms3_readmsr (&msr, "data/reference-testdata-int16.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Int32 */
  rv = ms3_readmsr (&msr, "data/reference-testdata-int32.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Steim-1 big endian */
  rv = ms3_readmsr (&msr, "data/reference-testdata-steim1.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  rv = ms3_readmsr (&msr, "data/reference-testdata-steim1-LE.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  /* Steim-2 big endian */
  rv = ms3_readmsr (&msr, "data/reference-testdata-steim2.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
  ...
  rv = ms3_readmsr (&msr, "data/reference-testdata-steim2-LE.mseed2", NULL, NULL, flags, 0);
  CHECK (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  REQUIRE (msr != NULL, "ms3_readmsr() did not populate 'msr'");
  REQUIRE (msr->datasamples != NULL, "ms3_readmsr() did not populate 'msr->datasamples'");
...
TEST (read, byterange)
{
  ...
  rv = ms3_readmsr (&msr, "data/testdata-oneseries-mixedlengths-mixedorder.mseed3@9428-9967", NULL, NULL, flags, 0);
  ...
  rv = ms3_readmsr (&msr, "data/testdata-oneseries-mixedlengths-mixedorder.mseed2@9344-9855", NULL, NULL, flags, 0);
  REQUIRE (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  CHECK (msr->numsamples == 112, "Byte range read, unexpected number of decoded samples");
  CHECK (msr->starttime == nstime, "Byte range read, unexpected record start time");
  ms3_readmsr(&msr, NULL, NULL, NULL, flags, 0);
...
TEST (read, selection)
{
  ...
  rv = ms3_readmsr_selection (&msfp, &msr, "data/testdata-3channel-signal.mseed3", NULL, NULL,
                              flags, selections, 0);
...
TEST (read, oddball)
{
  ...
  rv = ms3_readmsr (&msr, "data/testdata-detection.record.mseed2", NULL, NULL, flags, 0);
  REQUIRE (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  ...
  rv = ms3_readmsr (&msr, "data/testdata-unapplied-timecorrection.mseed2", NULL, NULL, flags, 0);
  REQUIRE (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  ...
  rv = ms3_readmsr (&msr, "data/testdata-no-blockette1000-steim1.mseed2", NULL, NULL, flags, 0);
  REQUIRE (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  ...
  rv = ms3_readmsr (&msr, "data/testdata-invalid-blockette-offsets.mseed2", NULL, NULL, flags, 0);
  REQUIRE (rv == MS_NOERROR, "ms3_readmsr() did not return expected MS_NOERROR");
  ms3_readmsr (&msr, NULL, NULL, NULL, flags, 0);
}
